...
impl QueueManager {
    ...
    fn format_identifier(original_id: &str) -> String {
        debug!("QueueManager::format_identifier: Formatting '{}'", original_id);
        ...
        if digit_part_str.len() == 1 {
            match digit_part_str.parse::<u32>() {
                Ok(num) => {
                    ...
                    trace!("QueueManager::format_identifier: Padded single digit: '{}' -> '{}'", original_id, formatted);
                    formatted
                },
                Err(_) => {
            ...
            trace!("QueueManager::format_identifier: No padding needed: '{}' -> '{}'", original_id, formatted);
            formatted
        }
    }
    ...
    fn remove_call_from_all_queues(&mut self, call_id: &str) -> Option<Call> {
        ...
        if let Some(pos) = self.completed_history.iter().position(|c| c.id == call_id) {
            ...
            let removed_from_history = self.completed_history.remove(pos)
        ...
        if let Some(pos) = self.skipped_history.iter().position(|c| c.id == call_id) {
            ...
            let removed_from_skipped = self.skipped_history.remove(pos)
                .expect("Call should exist at found position"); // Safe to unwrap because position was found.
            if found_call.is_none() {
    ...
    pub fn skip_current_call(&mut self) -> Option<Call> {
        info!("QueueManager::skip_current_call: Attempting. Current: {:?}", self.current_call.as_ref().map(|c| &c.id));
        if let Some(mut call_to_skip) = self.current_call.take() {
            info!("QueueManager::skip_current_call: Skipping call '{}'.", call_to_skip.id);
    ...
    pub fn complete_current_call(&mut self) -> Option<Call> {
        info!("QueueManager::complete_current_call: Attempting. Current: {:?}", self.current_call.as_ref().map(|c| &c.id));
        if let Some(mut call_to_complete) = self.current_call.take() {
            info!("QueueManager::complete_current_call: Completing call '{}'.", call_to_complete.id);
    ...
    pub fn get_current_call(&self) -> Option<&Call> {
        debug!("QueueManager::get_current_call: Returning current_call: {:?}", self.current_call.as_ref().map(|c| &c.id));
        self.current_call.as_ref()
    }
    ...
    pub fn get_completed_history(&self) -> &VecDeque<Call> {
        debug!("QueueManager::get_completed_history: Returning completed_history ({} calls).", self.completed_history.len());
    ...
    pub fn get_skipped_history(&self) -> &VecDeque<Call> {
        debug!("QueueManager::get_skipped_history: Returning skipped_history ({} calls).", self.skipped_history.len());
    ...
    fn enforce_max_completed_history_size(&mut self) {
        debug!("QueueManager: Enforcing max_completed_history_size (current: {}, max: {})", self.completed_history.len(), self.max_history_size);
    ...
    fn enforce_max_skipped_history_size(&mut self) {
        debug!("QueueManager: Enforcing max_skipped_history_size (current: {}, max: {})", self.skipped_history.len(), self.max_skipped_history_size);
...
mod tests {
    ...
    fn test_format_identifier_logic() {
        assert_eq!(QueueManager::format_identifier("A1"), "A01", "Should pad single digit 'A1' to 'A01'");
        assert_eq!(QueueManager::format_identifier("a1"), "A01", "Should uppercase and pad 'a1' to 'A01'");
        assert_eq!(QueueManager::format_identifier("B10"), "B10", "Should not pad multi-digit 'B10'");
        assert_eq!(QueueManager::format_identifier(""), "", "Should return empty string for empty input");
        // Additional tests for robustness:
        assert_eq!(QueueManager::format_identifier("Z9"), "Z09", "Should pad single digit 'Z9' to 'Z09'");
        assert_eq!(QueueManager::format_identifier("c123"), "C123", "Should uppercase and not pad 'c123'");
    ...
    fn test_add_call_sequence_t1_to_t4_behavior_and_uniqueness() {
        ...
        assert!(manager.get_current_call().is_none(), "Initially, current_call should be None");
        assert!(manager.get_completed_history().is_empty(), "Initially, completed_history should be empty");
        assert!(manager.get_skipped_history().is_empty(), "Initially, skipped_history should be empty");

        info!("Starting T2: Call A100, 4.");
        // T2: Add Call A100, 4
        manager.add_call("A100".to_string(), "4".to_string());
        let current = manager.get_current_call().expect("A100 should be current call");
        assert_eq!(current.id, "A100", "Current call ID should be A100");
        assert!(manager.get_completed_history().is_empty(), "Completed history should be empty after first call");
        assert!(manager.get_skipped_history().is_empty(), "Skipped history should be empty");

        info!("Starting T3: Call A101, 4.");
        // T3: Add Call A101, 4 (A100 should move to completed)
        manager.add_call("A101".to_string(), "4".to_string());
        let current = manager.get_current_call().expect("A101 should be current call");
        assert_eq!(current.id, "A101", "Current call ID should be A101");
        let completed = manager.get_completed_history();
        assert_eq!(completed.len(), 1, "Completed history should have 1 call");
        assert_eq!(completed[0].id, "A100", "The first completed call should be A100");
        assert!(manager.get_skipped_history().is_empty(), "Skipped history should still be empty");
        ...
        let current = manager.get_current_call().expect("A102 should be current call");
        assert_eq!(current.id, "A102", "Current call ID should be A102");
        let completed = manager.get_completed_history();
        assert_eq!(completed.len(), 2, "Completed history should have 2 calls");
        assert_eq!(completed[0].id, "A100", "First completed call should remain A100");
        assert_eq!(completed[1].id, "A101", "Second completed call should be A101");
        assert!(manager.get_skipped_history().is_empty(), "Skipped history should still be empty");
        ...
        let current = manager.get_current_call().expect("A100 should now be the current call");
        assert_eq!(current.id, "A100", "Current call ID should be A100 after re-add");
        assert_eq!(current.location, "new_loc_for_A100", "Location for A100 should be updated");
        let completed = manager.get_completed_history(); // Expected: A102 (prev current) -> A101
        assert_eq!(completed.len(), 2, "Completed history should still have 2 calls"); // A102 and A101 are there
        assert!(completed.iter().find(|c| c.id == "A100").is_none(), "A100 should no longer be in completed history");
        assert_eq!(completed.iter().find(|c| c.id == "A102").expect("A102 should be in completed history").id, "A102");
        assert_eq!(completed.iter().find(|c| c.id == "A101").expect("A101 should be in completed history").id, "A101");
        
        assert!(manager.get_skipped_history().is_empty(), "Skipped history should remain empty");
    ...
    fn test_skip_current_call_uniqueness() {
        ...
        assert_eq!(manager.get_current_call().expect("S2 should be current").id, "S2");
        assert_eq!(manager.get_completed_history().front().expect("S1 should be in completed").id, "S1");
        assert!(manager.get_skipped_history().is_empty(), "Skipped history should be empty initially");

        info!("Skipping current call (S2).");
        manager.skip_current_call(); // S2 should move to skipped history
        assert!(manager.get_current_call().is_none(), "Current call should be None after skipping");
        assert_eq!(manager.get_skipped_history().front().expect("S2 should be in skipped history").id, "S2");
        assert_eq!(manager.get_completed_history().front().expect("S1 should still be in completed history").id, "S1"); // S1 remains
    ...
    fn test_add_to_skipped_directly_t5_and_uniqueness() {
        ...
        assert_eq!(manager.get_completed_history().front().unwrap().id, "A100");
        assert_eq!(manager.get_completed_history().back().unwrap().id, "A101");
        ...
        assert_eq!(manager.get_current_call().unwrap().id, "A102", "Current call should remain A102"); 
        assert_eq!(manager.get_completed_history().len(), 2, "Completed history should remain 2"); // A100, A101
        let skipped = manager.get_skipped_history();
        assert_eq!(skipped.len(), 1, "Skipped history should have 1 call");
        assert_eq!(skipped[0].id, "A104", "The skipped call should be A104");
        ...
        assert!(manager.get_current_call().is_none(), "Current call should be None after A102 moved to skipped"); 
        let skipped = manager.get_skipped_history();
        assert_eq!(skipped.len(), 2, "Skipped history should now have 2 calls"); 
        assert!(skipped.iter().any(|c| c.id == "A102" && c.location == "new_loc_A102"), "A102 should be in skipped with updated location");
        assert!(skipped.iter().any(|c| c.id == "A104"), "A104 should still be in skipped");
        ...
        assert_eq!(manager.get_completed_history().iter().find(|c| c.id == "A100"), None, "A100 should no longer be in completed history"); 
        let skipped = manager.get_skipped_history(); 
        assert!(skipped.iter().any(|c| c.id == "A100" && c.location == "new_loc_A100"), "A100 should be in skipped with updated location");
        assert_eq!(skipped.len(), 3, "Skipped history should now have 3 calls: A104, A102, A100.");
